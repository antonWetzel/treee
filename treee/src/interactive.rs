...
impl Interactive {
	...
	pub fn ui(&mut self, ui: &mut egui::Ui) -> InteractiveResponse {
		...
		if ui.button("Reset").clicked() {
			self.modus = Modus::Select;
		}
		match &self.modus {
			Modus::Select => ui.label("No segment selected"),
			Modus::Draw(idx) => ui.label(format!("Segment {} selected", idx)),
		};

		ui.add(egui::Slider::new(&mut self.draw_radius, 0.1..=10.0).logarithmic(true));
	...
	pub fn draw(&mut self, start: na::Point3<f32>, direction: na::Vector3<f32>) {
		let Modus::Draw(idx) = self.modus else {
			return;
		};
